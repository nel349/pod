...
import {
  NoWallet, nextPosting, PostingStopped, type Payment, type PostingStep, type SealedJob, type StepState,
} from "../state/index.ts";
...
export function usePostJob(market: MarketConfig): PostJobState {
  ...
  const payment = useRef<Payment>(undefined);
  const [shownPayment, setShownPayment] = useState<Payment>();
  const remember = (paid: Payment): void => { payment.current = paid; setShownPayment(paid); };
  ...
  return { post: posting.mutate, steps, payment: shownPayment };
}
